...
class Seq2Seq(nn.Module):
  ...
  def beam_search(self, input_tensor, input_lengths=None, ext_vocab_size=None, beam_size=10, *,
    ...
    results= [] #this contains the finished hypotheses
    backup_results = []
    step = 0
    while hypos and step < 2 * max_out_len and len(results) < beam_size:  # prevent infinitely generating punctuations
      ...
      step += 1
    if len(results) == 0:  # if no sequence ends with EOS within desired length, fallback to sequences
      results = hypos  # that are "truncated" at the end to max_out_len
...
def format_nicely(tokens: List[str], newline: str= '<P>', for_rouge: bool=False) -> str:
  ...
  for i, doc in enumerate(decoded_tokens):
    ...
    for word_idx in doc:
      ...
      if word_idx == vocab.EOS:
        break #We have reached the end of the sentence and hence can stop.
