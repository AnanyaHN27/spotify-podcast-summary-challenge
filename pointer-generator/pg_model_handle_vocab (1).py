  ...
  def load_embeddings(self, file_path):
    ...
    with open(file_path, encoding='utf-8') as f:
      for line in f:
        ...
        if idx is not None: #ie. we have the word
          coefs = np.array(line[1:], dtype='float32')
          if self.embeddings is None:
            self.embeddings = np.random.normal(np.zeros((vocab_size, len(coefs)))).astype(dtype)
            self.embeddings[self.PAD] = np.zeros(len(coefs))
          self.embeddings[idx] = coefs
          num_embeddings += 1
    return num_embeddings

  def __getitem__(self, item):
    #if type(item) is int: return self.index2word[item]
...
class Dataset(object):
  ...
  def build_vocab(self, src: bool=True, tgt: bool=True,
                  embed_file: str=None):
...
def train_batch(batch, model, criterion, optimizer, *,
                ...
                rl_ratio=1, vocab=None, show_cover_loss=False):
  ...
  if rl_ratio>0:
    ...
    baseline = model(input_tensor, saved_out=out, visualize=False,
                         ext_vocab_size=ext_vocab_size)
...
class Seq2SeqOutput(object):
  ...
  def __init__(self, encoder_outputs: torch.Tensor, encoder_hidden: torch.Tensor,
               decoded_tokens: torch.Tensor, loss=0,
...
class Seq2Seq(nn.Module):
  ...
  def forward(self, input_tensor, target_tensor=None, input_lengths=None, criterion=None, *,
              ...
              saved_out: Seq2SeqOutput=None, include_cover_loss: bool=False)\
          -> Seq2SeqOutput:
...
def decode_batch(batch, model, vocab, criterion=None, *, pack_seq=True,
  ...
  with torch.no_grad():
    input_tensor = batch["input"].to(DEVICE)
    if batch["target"] is None or criterion is None:
      target_tensor = None
    else:
      target_tensor = batch["target"].to(DEVICE)
    out = model(input_tensor, target_tensor, input_lengths, criterion,
...
def rouge(target, *preds):
  """
  Target: list of list of strings
  Predictions: list of list of strings
  Returns: List of dicts mapping from strings to floats
  """
  scores = []
...
def eval_batch(batch, model: Seq2Seq, vocab: Vocab, criterion=None, *, pack_seq=True,
  ...
  examples = batch[0]
  gold_summaries = [ex[1] for ex in examples]
  scores = rouge(gold_summaries, decoded_batch)
  return out.loss_value, scores[0]['f']


def eval_batch_output(tgt_tensor_or_tokens, vocab: Vocab,
                      oov_dict: OOVDict, *pred_tensors: torch.Tensor):
  """
...
def eval_bs_batch(batch, model: Seq2Seq, vocab: Vocab, *, pack_seq=True, beam_size=4,
  ...
  file_content = [format_tokens(decoded_batch[0]), format_tokens(batch["examples"][0][1]), format_tokens(batch["examples"][0][0])]
...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq):
  test_gen = test_set.generator(1, vocab, None, True)
  n_samples = int(1 * len(test_set.pairs))
  ...
  for i in prog_bar:
    batch = next(test_gen)
    scores, file_content = eval_bs_batch(batch, model, vocab, pack_seq=True,
                                         beam_size=4,
                                         min_out_len=60,
                                         max_out_len=100
                                         )
  ...
  df.to_csv("/content/drive/MyDrive/Dissertation/data/pg_results.csv")

if __name__ == "__main__":

  filename = "/content/drive/MyDrive/Dissertation/data/pickled_for_colab.csv"
  dataset = Dataset(filename)
  v = dataset.build_vocab()
  m = Seq2Seq(v)
  train_gen = dataset.generator(1, v, v, True)
  d = Dataset("/content/drive/MyDrive/Dissertation/data/pickled_for_colab.csv")
  eval_bs(d, v, m)